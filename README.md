...
> **Claude Code を使うなら、手順1の画像配置だけ手でやって、あとは `/new-wallpaper <slug>` で
> 手順2〜4がまとめて通ります。** 以下は中で何が起きているかの内訳です。
...
トップは **`date`（ISOタイムスタンプ）の降順＝新着が上**で表示されます。`add-wallpaper` は追加時に
現在時刻を自動で入れます（同じ日に複数追加しても時刻で新着が上に来ます。`--date 2026-07-22` のように
日付だけで上書きも可）。並び順を変えたいときは、その作品の `date` を編集するだけです。`date` が無い
作品は一番下に回ります。
...
| コマンド            | 役割                                                                      |
|---------------------|---------------------------------------------------------------------------|
| `/new-wallpaper <slug>` | **新規作品はこれ1本。** `originals/<slug>/` の画像から、サムネ生成 → JSON登録 → モック生成 → R2アップロード → 作品ページ・sitemap 生成 → 確認のうえ commit/push まで通す。タイトル等は途中で聞かれる。 |
| `/upload-r2 <slug>` | **既存作品の画像差し替え用。** R2 に上げ直し、ページと sitemap を再生成して公開まで。未登録なら `/new-wallpaper` を案内して中断。 |
